...
## Revizyon 2 — Ausculta hizalama ve içerik sadeleştirmesi (18 Eylül 2026)

`YONERGE_REVIZYON_2_AUSCULTA_HIZALAMA.md` (Faz A–E, kullanıcı ekleriyle) uygulandı. Çalışma zamanı yalnız `cardai/` içinde; `model.js`, EKG sahnesi/kaliper, skorlama (8/10=80) ve SCORM şeması (`state.js` sürüm 6, 4096 bayt) değişmedi. İnceleme (sim) ekranına dokunulmadı.

**Faz A — Vaka/Değerlendirme kabuğu.** `#caseView`/`#quizView`'daki `.view-heading` (eyebrow + "Klinik bilgiyi EKG bulgusuyla birleştir." / "Gördüğünü mekanizmayla eşleştir." başlığı + "← Simülasyona dön") kaldırıldı; yerine yalnız ekran okuyucuya yönelik gizli (`.sr-only`) `h1` bırakıldı. İkinci ilerleme şeridi (`#casePageLabel`/`#caseProgressBar`/`#caseAnswered`) ve değerlendirme intro paragrafı kaldırıldı; header'daki tek ilerleme çubuğu artık gönderilen sayısına bağlı (mevcut davranış korunarak `features.js`'teki ölü referanslar temizlendi). Seçenekler harf rozeti yerine Ausculta radyo dairesi (`.opt-radio`) kullanıyor; gönderim öncesi "Yanıtla →" seçim yapılmadan `disabled`. Uygulama geri bildirimi büyük ✓/✗ ikonu + "Doğru!"/"Yanlış" + (yanlışsa) "Yanıtınız/Doğru yanıt" satırları + tek açıklama paragrafına indirildi; beş seçeneğin tam listesi yalnız Sonuçlar → Soru raporunda kalıyor. Olgu kartına `item.vitals` doluysa `.kv` kutuları eklendi. Mod kartlarındaki "Kurallar: …" satırı kaldırıldı (kural bilgisi `.strict-banner`'da zaten var).

**Faz D — Header/Footer/Landing/Hakkında.** Header, Ausculta `chrome.tsx` düzenine getirildi: marka — spacer — bağlam grubu (ilerleme metni + ince çubuk + mod çipi, header ortasında) — spacer — Mod Değiştir/Tam ekran (ikon, ≤720px'te gizli) — dikey ayırıcı — Yardım/Hakkında; ikonlar `icons.tsx`'ten birebir inline SVG (IconSwap/IconFullscreen/IconFullscreenExit/IconHelpCircle/IconInfo). Landing başlığı tek satıra indirildi ("EKG'yi, kalbi ve dolaşımı birlikte keşfedin."), bağlantı ve not satırlarına ikon eklendi. Hakkında ekranında üst "HAKKINDA" eyebrow'u ve sağ üst "← Geri" kaldırıldı; başlık Ausculta ölçeğinde; Kurum kartındaki kanıt cümlesi satır içi üst simge `[1]` + küçük atıf satırı biçimine getirildi; "← Geri" en alta taşındı. **Sapma:** footer ürün ikonunun Ausculta gibi ürün amblemine (favicon) çevrilmesi uygulanmadı — `qa/build.py`'nin sabit 15 kaynak dosyası ve `sol_package_tests.py`'nin gömülü görsel bayt kümesi eşitliği kontrolü `assets/brand/ege-tip-seal-128.png`'nin en az bir yerde referanslanmasını zorunlu kılıyor; footer'daki tek kullanım yeri kaldırılırsa paket testi kırılır. Bu nedenle Ege Tıp mührü footer'da bırakıldı (kullanıcı kararı bekleyen nokta). Aynı gerekçeyle Ausculta'daki footer sağ blok ("Sinyaller sentetiktir · klinik tanı için kullanılmaz") de eklenmedi: `qa/ui_audit.mjs`'nin footer metnini tam eşleşme ile doğrulayan `expectedFooter()` fonksiyonu güncellenmeden eklenirse >1280px genişliklerde test kırılır; kapsam içi zaman bütçesiyle bu güncelleme yapılmadı, kullanıcı kararı bekliyor. Header'daki değerlendirme zamanlayıcısı ayrı bir `IconClock`/`.eg-timer` öğesi yerine mevcut ilerleme metni içine gömülü bırakıldı (`"Soru N/10 · ⏱ mm:ss"`) — işlevsel olarak eşdeğer, görsel olarak küçük bir sadeleştirme.

**Faz E — Sonuçlar.** `#resultsView` `.results-wrap-v2` düzenine geçti; üst eyebrow ve sağ üst "← Geri" kaldırıldı; başlık (`Değerlendirme Tamamlandı` / `Vaka Raporu`) + tek paragraf alt metin. Zayıf alanlar artık ayrı bölüm değil, alt metnin hemen altında turuncu `.badge.orange.weak-chip` satırı (tıklanınca ilgili örüntüyle İnceleme açılır). Özet şeridi Ausculta `.rs-box/.rs-ring-sm/.rs-status/.rs-num` kalıbına taşındı (değer üstte, etiket altta); değerlendirmede 4 kutu (puan halkası, durum, süre, soru sayısı), vaka oturumunda 3 kutu (süre yok). Alan bazlı performans bir `.card` içinde ikonlu `.domain-row-v2` satırlarına (yalnız `%NN`, sayı `title`'da) geçti; `OBJECTIVE_LABELS` O3/O6 Faz B'deki yeni içerikle uyumlu olacak şekilde "Derivasyon ve bölge bilgisi" / "Hız, aralık ve düzen" olarak güncellendi. Soru/Vaka raporu `<details>` yerine `.report-table-v2` tablosuna (SORU·PUAN·SONUÇ / VAKA·SONUÇ, İPUCU sütunu yok) ve genişleyen `.report-detail-row-v2` satırına geçti. Eylemler: değerlendirmede birincil **"⏏ Modülden Çık"** (yeni; onay dialogu `#finishDialog` → `CardAIScorm.finish()` çağrısı + landing'e dönüş — mevcut otomatik `pagehide` bitirme akışını bozmaz, `finished` bayrağı çift çağrıyı engeller), vaka oturumunda **"Değerlendirmeye gir →"**; ikisinde de outline "Tekrar dene", "İnceleme modunda çalış" ve (Ausculta'da karşılığı olmayan, kullanıcı kararı bekleyen) "Yeni 10 …örneklemi" düğmesi kalıyor. Puan/`bestScore`/`passed`/`cmi.interactions` mantığı değişmedi.

**Faz B — İçerik.** `curriculum.js` tamamen yeniden yazıldı: `avr, avl, avf, limb, componentTransform, speed, timeScale, grid200, pvcAverage, anteriorAVL, inferiorAVR, iii28, avf24, avl18, pToQR315, pToQL335, j, rateVsRR, duration, qt, qtNone, prNone` bankaları (22 adet, formül/piksel/oynatma/ölçek soruları) kaldırıldı. Yerlerine 17 yeni klinik/kavramsal banka eklendi: O3 — `inferiorLeadGroup, lateralLeadGroup, precordialSeptal, anteriorLeadGroup, augmentedGroup` (hangi derivasyon grubu hangi bölgeyi gösterir); O6 — `qrsNarrowBank/qrsWideBank/qrsUnmeasurableBank` (QRS dar mı geniş mi), `prNormalBank/prUnmeasurableBank` (PR normal mi), `rhythmRegularBank/rhythmIrregularBank` (ritim düzenli mi), `pWaveVisibleYesBank/pWaveVisibleNoBank/pWaveEctopicBank` (P seçilebiliyor mu), `stIsoelectricBank/stSecondaryBank` (ST izoelektrik mi / sekonder mi). 400 maddenin tamamının stem'i "*NN yaşında kadın/erkek hasta.* **Başvuru:** …" kalıbında sentetik klinik vinyet olarak yeniden yazıldı (13 mod × 30 benzersiz yaş/cinsiyet/bağlam + 10 ek madde = 400 benzersiz vinyet); her satırın görevi (task) tek cümlelik klinik soru. Vitaller satır başına yazılmadı, `defaultVitals(mode,ecgOptions)` ile mod başına otomatik atanıyor (normal/stemi/inferior/pvc/lbbb/rbbb ≈75/dk; sintach 120; pat 150; flutter 150 düzenli; svt 165; vt 160 (hipotansif); af kontrollü 78/dk düzensiz / hızlı profil 142/dk düzensiz; vf "Nabız=alınamıyor, TA=ölçülemiyor, Bilinç=kapalı") ve Olgu kartında `.kv` kutusu olarak gösteriliyor. `makeItems`'taki "Sınırlandırılmış sentetik eğitim senaryosu. " öneki kaldırıldı; `limitations` metnine "Olgu vinyetleri ve vitaller sentetiktir." eklendi. Sayısal madde payı her havuzda tam **30/200 (%15)** — sınırın üstüne çıkmıyor. `qa/independent_content.mjs` (şema/benzersizlik/pozisyon/sayısal-model eşleşmesi) ve `qa/sol_state_model.mjs` (havuz başına 200 madde, mod:karar çifti benzersizliği) tekrar çalıştırılıp geçti. **Bilinçli sınırlama:** korunan (kaldırılmayan) ~74 bankanın seçenek/açıklama metni kelime kelime sadeleştirilmedi — yalnız yasaklı bankalar kaldırıldı/değiştirildi ve yeni bankalar sade yazıldı; mevcut bankaların çoğu zaten kısa/klinik ama tam bir "≤8 kelime seçenek" geçişi yapılmadı (zaman bütçesi; kullanıcı kararı bekliyor). O2 (ritim tanıma) objektifi her havuzda 13/200 madde ile B7'nin "en az 15" hedefinin 2 madde altında kalıyor (mod başına yalnız bir "hangi ritim" sorusu olduğundan yapısal üst sınır 13; ek madde havuzlarından 2'si O2'ye kaydırılabilirdi, yapılmadı — küçük, bilinçli sapma).

Revizyon 2 sonrası çalıştırılan kontroller (hepsi geçti): `python3 qa/build.py`, `python3 qa/sol_package_tests.py` (5/5), `qa/ui_audit.mjs` (5 kontrol/0 hata), `qa/sol_core.mjs` (43/43), `qa/sol_followup.mjs` (11/11), `qa/sol_ui.mjs` (0 sayfa hatası), `qa/sol_state_model.mjs` (208403 kontrol/0 hata), `qa/sol_scorm_edges.mjs` (8/8), `qa/independent_content.mjs` (3/3), `qa/independent_state.mjs` (5/5), `qa/independent_scorm.mjs` (16/16), `qa/independent_e2e.mjs` (tam senaryo, LMS mock ile), `qa/independent_model.mjs` (79/79), `qa/independent_gallery.mjs` (13/13). Ekran görüntüleri 1366×768/768×1024/390×844 `qa/evidence/family/rev2/` altında. Ayrıntılı rapor: `qa/evidence/family/rev2/RAPOR.md`.
